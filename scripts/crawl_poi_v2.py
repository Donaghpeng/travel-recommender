# -*- coding: utf-8 -*-
"""
crawl_poi_v2.py — 高德 POI 爬虫（使用 requests 库，如果有的话）
回退到 urllib（但增加重试和更长超时）
"""
...
CACHE_DIR = os.path.join(BASE_DIR, ".amap_cache_v2")
os.makedirs(RAW_DIR, exist_ok=True)
os.makedirs(CACHE_DIR, exist_ok=True)
OFFSET = 25
MAX_PAGES = 20  # 最多 20 页 = 500 POI
...
# 尝试导入 requests（第三方库更稳定）
try:
    import requests as _req_lib
    HAS_REQUESTS = True
    log("[网络] 使用 requests 库")
except ImportError:
    _req_lib = None
    HAS_REQUESTS = False
    log("[网络] requests 不可用，回退 urllib")

def _fetch(url, timeout=30):
    """统一网络请求函数"""
    if HAS_REQUESTS:
        resp = _req_lib.get(url, headers={"User-Agent": "Mozilla/5.0"}, timeout=timeout)
        resp.encoding = "utf-8"
        return resp.text
    else:
...
def _amap_search(city, keyword, page=1):
    ...
    for attempt in range(2):
        try:
            ...
            if attempt == 0:
                log(f"  [RETRY] {city}/{keyword} p{page}: {e}")
                time.sleep(3)
            else:
...
def main():
    ...
    raw_path = os.path.join(RAW_DIR, "poi_raw_v2.json")
    ...
    if stats:
        for p, n in sorted(stats.items(), key=lambda x: -x[1]):
            log(f"  {p}: {n}")
    log("\n下一步: python scripts/merge_poi_data.py --source data/.raw_new/poi_raw_v2.json 合并")
